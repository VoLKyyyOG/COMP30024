...
class GameObject:
    ...
    def update(self, colour, action):
        ...
        if action in available_actions:
            ...
            for player_code in self.score:
                self.score[player_code] = get_score(self.state, player_code)
    ...
    def _turn_detect_draw(self):
        ...
        if game_drawn(self.state):
            self.drawmsg = "game state has tied due to rules."
            ...
            if self.drawmsg == "":
                ...
                for colour, score in self.score.items():
                    if score > hiscore:
                        winner = colour
                        hiscore = score
                result = f"winner: {CODE_TO_NAME[winner]}"
            else:
